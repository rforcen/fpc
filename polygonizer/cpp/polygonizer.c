 ...
 * http::www.unchainedgeometry.com/jbloom/papers/polygonize.pdf
 * by Jules Bloomenthal, jules@bloomenthal.com
 * in "Graphics Gems IV", Academic Press, 1994 */

/* implicit.c
 *     an implicit surface polygonize, translated from Mesa
...
/* A Brief Explanation
 * The code consists of a test program and the polygonize itself.
 *
 * In the test program:
 * torus(), sphere() and blob() are primitive functions used to calculate
 *    the implicit value at a point (x,y,z)
 * triangle() is a 'callback' routine that is called by the polygonize
 ...
 *
 * The main data structures in the polygonize represent a hexahedral lattice,
 ...
 * in hash tables.
 *
 * The client entry to the polygonize is polygonize(), called from main().
 * This routine first allocates memory for the hash tables for the cube centers,
 * corners, and edges that define the polygonizing lattice. It then finds a
 start
...
#define RES 40 /* # converge iterations    */
...
double x, y, z;
{
  return 4.0 - sphere(x + 1.0, y, z) - sphere(x, y + 1.0, z) -
         sphere(x, y, z + 1.0);
}
...
int triangle(i1, i2, i3, vertices) int i1, i2, i3;
VERTICES vertices;
{
  gvertices = vertices;
  gntris++;
...
/* triangle2: called by polygonize() for each triangle; write to triangles
 * buffer */

int triangle2(i1, i2, i3, vertices) int i1, i2, i3;
VERTICES vertices;
{
  TRIANGLE t = {i1, i2, i3};
  if (!glefthanded) {
...
char *polygonize(function, bounds, size, x, y, z) double (*function)();
double bounds;
double size, x, y, z;
{ return c_polygonize(function, size, (int)bounds, x, y, z, triangle2, TET); }
#ifdef RUNNABLE
main() {
  int i;
  char *err, *polygonize();
  ...
  fprintf(stdout, "comment polygonize generated\n");
  if ((err = polygonize(blob, 30, 0.05, 0., 0., 0., triangle2, TET)) != NULL) {
    fprintf(stdout, "%s\n", err);
    exit(1);
  }
  ...
  for (i = 0; i < gvertices.count; i++) {
    VERTEX v;
    v = gvertices.vertex[i];
    fprintf(stdout, "%f %f %f %f %f %f\n", v.position.x, v.position.y,
            v.position.z, v.normal.x, v.normal.y, v.normal.z);
  }
  for (i = 0; i < gtriangles.count; i++) {
    TRIANGLE t;
    t = gtriangles.vertex[i];
 ...
 *		 vertices.vertex[i].position.x, .y, and .z
...
char *c_polygonize(function, size, bounds, x, y, z, triproc,
                   mode) double (*function)(),
    size, x, y, z;
...
void testface(i, j, k, old, face, c1, c2, c3, c4, p) CUBE *old;
PROCESS *p;
...
PROCESS *p;
{
  ...
  c->i = i;
  c->x = p->start.x + ((double)i - .5) * p->size;
  c->j = j;
  c->y = p->start.y + ((double)j - .5) * p->size;
  c->k = k;
  c->z = p->start.z + ((double)k - .5) * p->size;
  for (; l != NULL; l = l->next)
...
PROCESS *p;
{
  ...
  switch (index) {
  case 1:
    return p->triproc(e5, e6, e3, p->vertices);
  case 2:
    return p->triproc(e2, e6, e4, p->vertices);
  case 3:
    return p->triproc(e3, e5, e4, p->vertices) &&
           p->triproc(e3, e4, e2, p->vertices);
  case 4:
    return p->triproc(e1, e4, e5, p->vertices);
  case 5:
    return p->triproc(e3, e1, e4, p->vertices) &&
           p->triproc(e3, e4, e6, p->vertices);
  case 6:
    return p->triproc(e1, e2, e6, p->vertices) &&
           p->triproc(e1, e6, e5, p->vertices);
  case 7:
    return p->triproc(e1, e2, e3, p->vertices);
  case 8:
    return p->triproc(e1, e3, e2, p->vertices);
  case 9:
    return p->triproc(e1, e5, e6, p->vertices) &&
           p->triproc(e1, e6, e2, p->vertices);
  case 10:
    return p->triproc(e1, e3, e6, p->vertices) &&
           p->triproc(e1, e6, e4, p->vertices);
  case 11:
    return p->triproc(e1, e5, e4, p->vertices);
  case 12:
    return p->triproc(e3, e2, e4, p->vertices) &&
           p->triproc(e3, e4, e5, p->vertices);
  case 13:
    return p->triproc(e6, e2, e4, p->vertices);
  case 14:
    return p->triproc(e5, e3, e6, p->vertices);
  }
  return 1;
}
...
static int corner1[12] = {LBN, LTN, LBN, LBF, RBN, RTN,
                          RBN, RBF, LBN, LBF, LTN, LTF};
static int corner2[12] = {LBF, LTF, LTN, LTF, RBF, RTF,
                          RTN, RTF, RBN, RBF, RTN, RTF};
...
setedge(table, i1, j1, k1, i2, j2, k2, vid) EDGELIST *table[];
...
addtovertices(vertices, v) VERTICES *vertices;
VERTEX v;
{
  if (vertices->count == vertices->max) {
    ...
    for (i = 0; i < vertices->count; i++)
      new[i] = vertices->vertex[i];
    if (vertices->vertex != NULL)
      free((char *)vertices->vertex);
    vertices->vertex = new;
  }
  vertices->vertex[vertices->count++] = v;
}

/* addtotriangles: add t to sequence of triangles */

int addtotriangles(triangles, t) TRIANGLES *triangles;
TRIANGLE t;
{
  if (triangles->count == triangles->max) {
    ...
    for (i = 0; i < triangles->count; i++)
      new[i] = triangles->trig[i];
    if (triangles->trig != NULL)
      free((char *)triangles->trig);
    triangles->trig = new;
  }
  triangles->trig[triangles->count++] = t;
  return 1;
}

/* vnormal: compute unit length surface normal at point */

vnormal(point, p, v) POINT *point, *v;
PROCESS *p;
{
...
void converge(p1, p2, v, function, p) double v;
double (*function)();
POINT *p1, *p2, *p;
{
  int i = 0;
