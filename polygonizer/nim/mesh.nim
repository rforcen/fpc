...
import strformat, math, ctm
import algorithm, lzma
...
# mg2'like compression support

type 
  VertexComponents = enum Position, Normal, Color, Uv
  SortedIndex = object
    x : cfloat
    index : cuint

proc vcomp(v:Vertex, vc:VertexComponents):vec3=
  case vc:
    of Position:  v.pos
    of Normal:  v.norm
    of Color:  v.color
...
proc vtos(m:Mesh, vc:VertexComponents) : seq[cfloat] =
  for v in m.shape: result.add v.vcomp vc

proc `<`(a,b:SortedIndex):bool = a.x < b.x

proc sortedIndex(m:Mesh, vc : VertexComponents):seq[SortedIndex]=
  for i,v in m.shape.pairs:
    for j,x in v.pos.pairs:
      result.add SortedIndex(x:x, index:(i*3+j).cuint)
  result.sort

proc get_index(si:seq[SortedIndex]):seq[cuint]=
  for s in si: result.add s.index

# interleave seq[SortedIndex].index
proc div_table4(s:seq[uint8]):array[4,int]=
  for i in 0..3: result[i]=(i*s.len) div 4
  
proc interleave(si:seq[SortedIndex]):seq[uint8]=
  result = newSeq[uint8](si.len * cuint.sizeof)
  let dt = result.div_table4
  for i in 0..si.high:
    let a4 = cast[array[4,uint8]](si[i].index)
    for j in 0..3: result[i + dt[j]] = a4[3-j]

proc interleave(sf:seq[cfloat]):seq[uint8]=
  result = newSeq[uint8](sf.len * sf[0].sizeof)
  let dt = result.div_table4
  for i in 0..sf.high:
    let a4 = cast[array[4,uint8]](sf[i])
    for j in 0..3: result[i + dt[j]] = a4[j]

proc deinterleave[T](s8:seq[uint8]):seq[T]=
  let dt = s8.div_table4

  for i in 0..<s8.len div 4:
    var a4:array[4,uint8]
    for j in 0..3: a4[j]=[s8[i+dt[j]]]
    result.add cast[T](a4)
...
when isMainModule:
  ...
  proc comp_mg2=
    ...
    echo "# vertex:", m.nvertex

    # conclusion: compressing interleaved cfloat not worth it
    for vc in [Position, Normal, Color, Uv]:
      echo "vc:", vc
      echo "org compress               :", m.vtos(vc).tostr.compress.nbytes

      # seq[cfloat] sorted vertex
      let si = m.sortedIndex vc
      let 
        sf = m.vtos(vc).sorted
        sfc = lzma.compress(sf.tostr)
      echo "sorted compress:", sfc.nbytes

      # interleave
      let 
        sinitl=si.get_index
        itl = si.interleave
        itlc = lzma.compress(itl.tostr)
      echo "no itl compress:", sinitl.tostr.compress.len
      echo "itl:", itl.nbytes, ", itlc:", itlc.nbytes
      echo "compressed (f+i):", sfc.nbytes + itlc.nbytes
  ...
  proc zm_test* =
    var mesh = CTMread "sh.ctm"
    mesh.ZMwrite "sh.zm"
    mesh.PLYwrite "sh.ply"
