...
type
  _fp80 = packed array [0..9] of byte;  // 80 bits : 10 bytes

  fp80 = packed record
    v: packed array [0..9] of byte;
  end;  // 80 bits : 10 bytes
  pfp80 = ^fp80;
  _pfp80 = ^_fp80;
...
begin
  ...
  if exi <> 0 then
    Result := format('%.*fe%d', [decimals, double(m), exi])
  else
    Result := format('%.*f', [decimals, double(m)]);
end;

function __fp80ToStr(x: fp80; decimals: integer): string;
const
  sqTab: array of _fp80 = ( // table of squares or 10: 1,10,100,10000...
